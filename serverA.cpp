...
int main(void)
{
    ...
    if ((rv = getaddrinfo(NULL, Server_A_UDP_PORT, &hints, &servinfo)) != 0) {
    ...
    cout << "Server A is up and running using UDP on port " << Server_A_UDP_PORT<<"."<<endl;
    ...
    while(1){
        ...
        cout << "The Server A has received " << count << " numbers."<<endl;
        
        cout << "The Server A has successfully finished the reduction " << type <<":";
        
        //prepare to send the results to AWS
        stringstream backend_total;
        backend_total<<"A"; //pack the server name into the string stream with the answer
